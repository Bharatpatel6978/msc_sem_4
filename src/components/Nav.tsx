...
function Nav({ user }: { user: IUser }) {
  ...
  const sideBar = menuOpen?createPortal(
    <AnimatePresence>
      ...
      className="fixed top-0 left-0 h-full w-[75%] sm:w-[60%] z-9999 bg-linear-to-b from-green-800/90 via-green-700/80 to-green-900/90 backdrop-blur-xl border-r border-green-400/20 shadow-[0_0_50px_-10px_rgba(0,255,100,0.3)] flex-col flex p-6 text-white"
      >

        <div className="flex justify-between items-center mb-2">
          <h1 className="font-semibold text-2xl tracking-wide text-white/90">Admin Pannel</h1>
        ...
        <div className="flex flex-col gap-3 font-medium mt-6">
          <Link href={"/admin/add-grocery"} className="flex p-2 items-center gap-3 rounded-lg bg-white/10 hover:bg-white/20 hover:pl-4 transition-all"><PlusCircle className="w-5 h-5" />Add Grocery</Link>
          <Link href={""} className="flex p-2 items-center gap-3 rounded-lg bg-white/10 hover:bg-white/20 hover:pl-4 transition-all"><Boxes className="w-5 h-5"/>view Grocery</Link>
          <Link href={"/admin/manage-orders"} className="flex p-2 items-center gap-3 rounded-lg bg-white/10 hover:bg-white/20 hover:pl-4 transition-all" ><ClipboardCheck className="w-5 h-5" />Manage Orders</Link>
        </div>

        <div className="my-5 border-t border-white/20"></div>
        <div className="flex cursor-pointer items-center gap-3 text-red-300 font-semibold mt-auto hover:bg-red-500/20 p-3 rounded-lg transition-all" onClick={async ()=>await signOut({callbackUrl:"/"})}>
  ...
  return (
    <div className="w-[95%] fixed top-4 left-1/2 -translate-x-1/2 bg-linear-to-r from-green-500 to-green-700 rounded-2xl shadow-lg shadow-black/30 flex justify-between items-center h-20 px-4 md:px-8 z-50">
      <Link
        href={"/"}
        className="text-white font-extrabold text-2xl sm:text-2xl md:text-3xl tracking-wide hover:scale-105 transition-transform"
      >
        SK MART
      </Link>
      ...
      <div className="flex items-center gap-3 md:gap-6 relative">
        {user.role == "user" && (
          <>
            <div
              className="bg-white rounded-full w-11 h-11 flex items-center justify-center shadow-md hover:scale-105 cursor-pointer transition md:hidden"
        ...
        <div className="hidden md:flex items-center gap-4">
          ...
          <Link href={"/admin/manage-orders"} className="flex items-center gap-2 bg-white text-green-700 font-semibold px-4 py-2 rounded-full hover:bg-green-100 transition-all"><Clipboard className="w-5 h-5" />Manage Orders</Link>
        </div>
        ...
        <div className="md:hidden bg-white rounded-full w-10 h-10 flex items-center justify-center shadow-md" onClick={()=>setMenuOpen(prev=>!prev)}>
          <Menu className="text-green-600 cursor-pointer w-6 h-5"/>
        </div>
        </>}
          ...
          <AnimatePresence>
            {open && (
              ...
              >
                ...
                <Link
                  href={"/user/my-orders"}
                  className="flex items-center gap-2 px-3 py-2 hover:bg-green-50 rounded-lg text-gray-700 font-medium"
          ...
          <AnimatePresence>
            {searchBarOpen && (
              ...
              >
                ...
                <button onClick={() => setSearchBarOpen(false)} className="cursor-pointer">
                  <X className="text-gray-500 cursor-pointer w-5 h-5" />
                </button>
              </motion.div>
            )}
          </AnimatePresence>
