...
import { g, input } from "motion/react-client";
...
function Checkout() {
  ...
  const { subTotal,deliveyFee,finalTotal,cartData } = useSelector((state: RootState) => state.cart);
  ...
  useEffect(()=>{
    ...
    const handleCod=async ()=>{
      ...
      try {
        const resut = await axios.post("/api/user/order",{
          userId: userData?._id,
  ...
  return (
    <div className="w-[92%] md:w-[80%] mx-auto py-10 relative">
      <motion.button
        whileTap={{ scale: 0.97 }}
        className="cursor-pointer absolute left-0 top-2 flex items-center gap-2 text-green-700 hover:text-green-800 font-semibold"
