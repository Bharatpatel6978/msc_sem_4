...
function CartPage() {
  const { cartData, subTotal,finalTotal,deliveyFee } = useSelector((state: RootState) => state.cart);
          ...
          <div className="lg:col-span-2 space-y-5">
            <AnimatePresence>
              {cartData.map((item, index) => (
                ...
                >
                  <div className="relative w-28 h-28 sm:w-24 sm:h-24 md:w-28 md:h-28 flex-shring-0 rounded-xl overflow-hidden bg-gray-50">
                    <Image
                      src={item.image}
                      alt={item.name}
                      fill
                  ...
                  <div className="flex items-center justify-center sm:justify-end gap-3 mt-3 sm:mt-0 bg-gray-50 py-2 rounded-full">
                    <button
                      onClick={() => dispatch(decreaseQuantity(item._id))}
                      className="cursor-pointer bg-white p-1.5 rounded-full hover:bg-green-100 transition-all border border-gray-200"
                    >
                  ...
                  <button
                    ...
                    className="cursor-pointer sm:ml-4 mt-3 sm:mt-0 text-red-500 hover:text-red-700 transition-all"
                  >
                    <Trash2 size={18} />
                  </button>
                </motion.div>
            ...
            <h2 className="text-lg sm:text-xl font-bold text-gray-800 mb-4">Order Summary</h2>
             <div className="space-y-3 text-gray-700 text-sm sm:text-base">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-green-700 font-semibold">₹{deliveyFee}</span>
              </div>
              <hr className="my-3" />
